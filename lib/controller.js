...
app.get('/startGame',function(req, res){
	var content;
	if(req.game && req.game.allPlayersJoined()){
		content = {gameStarted:true, url:'gamePage.html'};
		var croupier = new lib.Croupier(req.game.players);
		req.game.addCroupier(croupier);
	}
	else content = {gameStarted:false, url:'gamePage.html'};
	res.send(content);
});
...
app.get('/provideTable',function(req,res){
	var game = req.game, player;
	if(game){
		player = JSON.parse(req.cookies.id);
...
app.post('/sayUno',function(req,res){
	var player = req.game.croupier.getPlayerDetails(JSON.parse(req.cookies.id).id);
...
app.post('/cpu', function(req, res, next){
	...
	for(var i = 0; i < limit-1; i++){
		...
		const child = execFile('phantomjs', ['./lib/unobot.js', name], function(error, stdout, stderr){
		  if (error) {
		    throw error;
		  }
		  console.log(stdout);
...
var controller = function(game){
	return function(req,res){
		...
		if(player){
			...
			game = ld.find(games,function(game){
				return ld.find(game.players,{id:player.id});
			});
		}
		req.game = game;
		app(req,res);
